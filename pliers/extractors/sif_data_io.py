    ...
    for i in lines:
        ...
        if(len(i) > 0):
            ...
            if(len(i) == 2):
                ...
                print(i)
    for key, value in word2weight.items(): # changed by DG
        word2weight[key] = a / (a + value/N)
    return word2weight

def getWeight(words, word2weight):
    weight4ind = {}
    for word, ind in words.items(): # changed by DG
...
def seq2weight(seq, mask, weight4ind):
    ...
    for i in range(seq.shape[0]): # xrange -> range in python3 (debanjan)
